	...
	get '/drugs' do
		...
		for cpt in cpts do
			...
			if all_counts.length > 0
				...
				current_count = {:count => all_counts[-1].count,:date => nil} #this is going to be the one we append to the end of all_counts
			...
			cpt_hash = {:code => cpt.code, :name => cpt.drug.name, :all_counts => all_counts, :current_count => current_count, :vendor => cpt.drug.vendor} #we need to keep all_counts for graphing
			else
